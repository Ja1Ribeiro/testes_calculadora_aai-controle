describe('Aquisição de imovel', () => {
  //Garante que será efetuado um novo login antes de cada execução
  //E que será redirecionado para a pagina onde se encontra os produtos
  beforeEach(() => {
  ...
  it('Realiza a simulação de Aquisição de Imoveis Residenciais', () => {
    ...
    cy.get('.css-x82pny > :nth-child(1)')
      .click();
    cy.get('.MuiGrid-container > .MuiGrid-root > .MuiButtonBase-root')
      .click();
    ...
    cy.fixture('credentials').then((credentials) => {
      ...
      cy.get('button[type="button"]')
        .contains('CONHECER TODAS AS OFERTAS')
        .click();
      //Preenche os dados do formulario com os dados que estão no Json
      //Obs** Gostaria de ter otimizado o segundo formulario, porem cada campo de cada uma 
      //das três paginas possuiam ids diferentes.
      cy.get('#\\:r10\\:')
        .type(credentials.acquisition.name);
      cy.get('#\\:r11\\:')
        .type(credentials.acquisition.CPF);
      cy.get('#\\:r12\\:')
        .type(credentials.acquisition.cellphone);
      cy.get('#\\:r13\\:')
        .type(email);
      cy.get('#\\:r16\\:')
        .type(credentials.acquisition.indication);
      cy.get('#\\:r14\\:')
        .click();
      cy.get('#\\:r14\\:-option-0')
        .click();
      ...
      //Garante que o Alert de sucesso será exibido em tela após a finalização
  ...
  it('Realiza a simulação de Emprestimo para PF de um Imovel comercial', () => {
    cy.get('.css-x82pny > :nth-child(2)')
      .click();
    cy.get('.css-frij0f')
      .contains('PF')
      .click();
    cy.get('button[type="button"]')
      .contains('PRÓXIMA ETAPA')
      .click();
    ...
    cy.fixture('credentials').then((credentials) => {
      ...
      //Preenche os dados do formulario com os dados que estão no Json  
      cy.get('#\\:r12\\:')
        .type(credentials.loan.name);
      cy.get('#\\:r13\\:')
        .type(credentials.loan.CPF);
      cy.get('#\\:r14\\:')
        .type(credentials.loan.cellphone);
      cy.get('#\\:r15\\:')
        .type(email);
      cy.get('#\\:r18\\:')
        .type(credentials.loan.indication);
      cy.get('#\\:r16\\:')
        .click();
      cy.get('#\\:r16\\:-option-0')
        .click();
  ...
  it('Realiza a simulação de Consorcio para PJ de um Veiculo Pesado', () => {
    cy.get('.css-x82pny > :nth-child(3)')
      .click();
    cy.get('.css-frij0f')
      .contains('PJ')
      .click();
    ...
    cy.get('.MuiFormGroup-root > :nth-child(4)')
      .click();
    ...
    cy.fixture('credentials').then((credentials) => {
      ...
      //Preenche os dados do formulario com os dados que estão no Json
      cy.get('#\\:rv\\:')
        .type(credentials.consortium.name);
      cy.get('#\\:r10\\:')
        .type(credentials.consortium.CPF);
      cy.get('#\\:r11\\:')
        .type(credentials.consortium.cellphone);
      cy.get('#\\:r12\\:')
        .type(email);
      cy.get('#\\:r15\\:')
        .type(credentials.consortium.indication);
      cy.get('#\\:r13\\:')
        .click();
      cy.get('#\\:r13\\:-option-0')
        .click();
